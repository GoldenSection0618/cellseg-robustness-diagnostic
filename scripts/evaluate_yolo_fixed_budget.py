#!/usr/bin/env python
"""Evaluate the fixed-budget YOLO baseline with repository instance metrics."""
...
DEFAULT_CONF = 0.25
DEFAULT_IMGSZ = 512
METRICS_PATH = RESULT_SUBDIRS["supervised"] / "yolo_fixed_budget_metrics.csv"
SUMMARY_PATH = RESULT_SUBDIRS["supervised"] / "yolo_fixed_budget_eval_summary.csv"
FIGURE_PATH = FIGURES_DIR / "supervised_yolo_fixed_budget_eval_overlays.png"
...
def prepare_outputs(overwrite: bool) -> None:
    ...
    if existing and not overwrite:
        ...
        raise FileExistsError(f"Existing YOLO fixed-budget evaluation outputs found. Use --overwrite.\n{joined}")
    if overwrite:
...
def summarize(metrics: pd.DataFrame, metadata: pd.Series, conf: float, imgsz: int) -> pd.DataFrame:
    ...
    summary = {
        "protocol": "supervised_yolo_fixed_budget_eval",
        "images": len(metrics),
