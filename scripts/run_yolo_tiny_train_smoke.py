#!/usr/bin/env python
"""Run a tiny YOLO segmentation training smoke on the converted DSB2018 subset."""
...
DEFAULT_DATA = RESULT_SUBDIRS["supervised"] / "yolo_label_smoke" / "data.yaml"
RUN_NAME = "yolo_tiny_train_smoke_run"
METADATA_PATH = RESULT_SUBDIRS["supervised"] / "yolo_tiny_train_smoke_metadata.csv"
SUMMARY_PATH = RESULT_SUBDIRS["supervised"] / "yolo_tiny_train_smoke_summary.csv"
...
def prepare_outputs(overwrite: bool) -> Path:
    ...
    if existing and not overwrite:
        ...
        raise FileExistsError(f"Existing tiny training outputs found. Use --overwrite.\n{joined}")
    if overwrite:
        if run_dir.exists():
...
def main() -> None:
    ...
    metadata = {
        "protocol": "supervised_yolo_tiny_train_smoke",
