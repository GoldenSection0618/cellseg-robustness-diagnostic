#!/usr/bin/env python
"""Evaluate a YOLO label-budget diagnostic checkpoint with repository metrics."""
...
def metrics_path(budget: str) -> Path:
    return RESULT_SUBDIRS["supervised"] / f"yolo_label_budget_diagnostic_{budget}_metrics.csv"


def summary_path(budget: str) -> Path:
    return RESULT_SUBDIRS["supervised"] / f"yolo_label_budget_diagnostic_{budget}_eval_summary.csv"


def figure_path(budget: str) -> Path:
    return FIGURES_DIR / f"supervised_yolo_label_budget_diagnostic_{budget}_eval_overlays.png"


def prepare_outputs(budget: str, overwrite: bool) -> None:
    existing = [path for path in [metrics_path(budget), summary_path(budget), figure_path(budget)] if path.exists()]
    if existing and not overwrite:
        joined = "\n".join(str(path) for path in existing)
        raise FileExistsError(f"Existing YOLO label-budget evaluation outputs found. Use --overwrite.\n{joined}")
    if overwrite:
...
def save_overlay_figure(budget: str, examples: list[tuple[str, np.ndarray, np.ndarray, np.ndarray]]) -> None:
    columns = 4
    ...
    for ax in axes_array[len(examples) :]:
        ax.axis("off")
    fig.tight_layout()
    fig.savefig(figure_path(budget), dpi=160)
    plt.close(fig)
...
def summarize(metrics: pd.DataFrame, metadata: pd.Series, conf: float, imgsz: int) -> pd.DataFrame:
    ...
    summary = {
        "protocol": "supervised_yolo_label_budget_eval",
...
def main() -> None:
    args = parse_args()
    ensure_output_dirs()
    prepare_outputs(args.budget, args.overwrite)
    metadata = pd.read_csv(metadata_path(args.budget)).iloc[0]
    manifest = pd.read_csv(RESULT_SUBDIRS["supervised"] / "yolo_label_budget_diagnostic_manifest.csv")
    val_manifest = manifest[(manifest["budget"] == args.budget) & (manifest["split"] == "val")].copy()
    ...
    metrics_df.to_csv(metrics_path(args.budget), index=False)
    summary_df.to_csv(summary_path(args.budget), index=False)
    ...
    save_overlay_figure(args.budget, overlay_examples)

    print(f"Wrote {metrics_path(args.budget)}")
    print(f"Wrote {summary_path(args.budget)}")
    print(f"Wrote {figure_path(args.budget)}")
